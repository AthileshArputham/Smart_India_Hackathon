...
class Phase:
    ...
    def update_time(self, time):
        self._duration = time
        self._duration1 = 2000
        self._duration2 = 50000

        if time < self._duration1:
            self._duration += 2*self._duration1
        elif time > self._duration2:
            self._duration -= 2*self._duration2
...
class Logic:
    ...
    def return_phase(self):
        phase_results = []
        for p in self._phases:
            phase_results.append(p.time())
        return phase_results

    def set_stateDuration(self, time):
        for i in range(len(self._phases)):
            self._phases[i].update_time(time[i])
